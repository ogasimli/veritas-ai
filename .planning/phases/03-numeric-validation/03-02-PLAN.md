...
1. Create validator.py with ValidatorAgent:
   ...
   - Define VALIDATOR_INSTRUCTION constant:
     ...
     * For each FSLI with multiple values, write Python code to:
       - Extract the values
     ...
     * Output JSON array of validation results: [{fsli_name, check_type, expected, actual, is_valid, discrepancy, code_used}]
...
1. Create backend/app/schemas/validation.py:
   - Define ValidationResult Pydantic model:
     * fsli_name: str
     * check_type: str (e.g., "sum_check", "percentage_check", "yoy_change")
     * expected: float | None
     * actual: float | None
     * is_valid: bool
...
- Planner → Validator state flow established via {fslis}
</success_criteria>
