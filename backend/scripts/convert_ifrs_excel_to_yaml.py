...
def convert_excel_to_yaml():
    ...
    for row_num, row in enumerate(checklist_sheet.iter_rows(min_row=10, values_only=True), 10):
        ...
        if clean_disc and current_standard:
            ...
            requirement_title = clean_disc[:100] if len(clean_disc) <= 100 else clean_disc[:97] + "..."
    ...
    for std_code, std_data in sorted(standards.items(), key=lambda x: x[0]):
        print(f"  {std_code:10s} ({std_data['name'][:40]:40s}): {len(std_data['disclosures']):3d} disclosures")
